...
public class GameWorld extends Observable {
	...
	private void checkAlienCollisions(){					//check all aliens to see if they collided with anything
		...
		while(mainIt.hasNext()){
			GameObject curObj=mainIt.getNext();				//cur object
			IIterator secondIt = this.getGOIt();			//second iterator
			while(secondIt.hasNext()){
				GameObject otherObj=secondIt.getNext();		//other object	
				if(otherObj.returnType()!="Alien"){continue;}//otherObj will always be ALIEN
				if (curObj.equals(otherObj)){continue;}		//if main object is the same as the other object, skip this other object
				else{
					if (mainIt.getCurrent().collidesWith(otherObj)){mainIt.getCurrent().handleCollision((Alien) otherObj, allAiGameObj, this);}	//if curObj and otherObj are in each other space, handle the collision
					else{mainIt.getCurrent().removeObj(otherObj);}										//else remove otherObj from curObj collision list
				}//else
