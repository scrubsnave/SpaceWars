...
public class GameWorld extends Observable {
	...
	private boolean sound = false;//is sound ON or OFF
	private int worldHeight; //height of game window
	private int worldWidth;  //width of game window
	private Spaceship mainChar; //create one instance of our spaceship
	...
	public void init(){
		...
		mainChar = Spaceship.getInstance(worldWidth,worldHeight);
		for(int i =0;i<startingAstronauts;i++){
			allAiGameObj.add(new Astronaut(worldWidth,worldHeight));
		}		
		for(int i=0;i<startingAliens;i++){
			allAiGameObj.add(new Alien(worldWidth,worldHeight));
		}
	}
	...
	public void toAlien(){
		...
		int temp=(num.nextInt(this.getAlienRem())+1);	//randomize between 0 and number of aliens remaining
		IIterator it = this.getGOIt();	
		GameObject tempGameObj = new Alien(1,1);
		
		for(int i=0;i<temp;){  //this for loop iterates to the random alien
			if(it.hasNext()){
				tempGameObj=it.getNext();  //if it has next, set next to tempGameObj
	...
	public void toAstronaut(){
		...
		Random num = new Random();					//create a random number
		int temp=(num.nextInt(this.getAstroRem())+1);	//randomize between 0 and number of astronauts remaining
		IIterator it = this.getGOIt();
		GameObject tempGameObj = new Astronaut(1,1);
	...
	public void tick(){  //one tick of the game clock	
		//this.toMap(); //sends update to the console
		
		IIterator it = this.getGOIt();				
		while(it.hasNext()){((Opponent) it.getNext()).move();}		//move all the game objects
	...
	private void checkAlienCollisions(){					//check all aliens to see if they collided with anything
		...
		while(mainIt.hasNext()){
			mainIt.getNext();				//iterate cur object
			IIterator secondIt = this.getGOIt();			//second iterator
			while(secondIt.hasNext()){
				secondIt.getNext();		//iterate other object	
				if(secondIt.getCurrent().returnType()!="Alien"){continue;}//otherObj will always be ALIEN
				if (mainIt.getCurrent().equals(secondIt.getCurrent())){continue;}		//if main object is the same as the other object, skip this other object
				if (mainIt.getCurrent().collidesWith(secondIt.getCurrent())==true){mainIt.getCurrent().handleCollision((Alien) secondIt.getCurrent(), allAiGameObj, this);}	//if curObj and otherObj are in each other space, handle the collision
	...
	private void checkAllAstronautHealth() {
		IIterator it = this.getGOIt();
		while(it.hasNext()){
			GameObject tempGameObj=it.getNext();
